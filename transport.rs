use network_channels::{
    ...
    BitsClient, BitsServer,
};

use bevy::{app::AppExit, prelude::*};

use crate::{BitsClientPlugin, BitsServerPlugin};
    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            ...
            Self::update_system
                ...
                .run_if(resource_exists::<BitsClient>())
                .after(BitsServerPlugin::update_system),
        );
...
impl NetcodeServerPlugin {
    ...
    pub fn send_packets(mut transport: ResMut<NetcodeServerTransport>, mut server: ResMut<BitsServer>) {
        transport.send_packets(&mut server);
    }

    pub fn disconnect_on_exit(exit: EventReader<AppExit>, mut transport: ResMut<NetcodeServerTransport>, mut server: ResMut<BitsServer>) {
        if !exit.is_empty() {
    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            ...
            Self::update_system
                ...
                .run_if(resource_exists::<BitsClient>())
                .after(BitsClientPlugin::update_system),
        );
        app.add_systems(
            PostUpdate,
            (Self::send_packets, Self::disconnect_on_exit)
