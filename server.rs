...
use network_core::{NetcodeServer, ServerResult, NETCODE_KEY_BYTES, NETCODE_MAX_PACKET_BYTES, NETCODE_USER_DATA_BYTES};

use crate::server::BitsServer;
...
impl NetcodeServerTransport {
    ...
    // should use [BitsServer::disconnect_all][crate::BitsServer::disconnect_all] otherwise.
    pub fn disconnect_all(&mut self, server: &mut BitsServer) {
    ...
    pub fn update(&mut self, duration: Duration, server: &mut BitsServer) -> Result<(), NetcodeTransportError> {
    ...
    pub fn send_packets(&mut self, server: &mut BitsServer) {
                ...
                match self.netcode_server.generate_payload_packet(client_id, &packet) {
                    Ok((addr, payload)) => {
                        if let Err(e) = self.socket.send_to(payload, addr) {
                            println!("Failed to send packet to client {client_id} ({addr}): {e}");
                            continue 'clients;
                        }
                    }
                    Err(e) => {
                        println!("Failed to encrypt payload packet for client {client_id}: {e}");
                        continue 'clients;
                    }
                }
            }
...
fn handle_server_result(server_result: ServerResult, socket: &UdpSocket, reliable_server: &mut BitsServer) {
    let send_packet = |packet: &[u8], addr: SocketAddr| {
        if let Err(err) = socket.send_to(packet, addr) {
            println!("Failed to send packet to {addr}: {err}");
        }
    };
    ...
    match server_result {
        ...
        ServerResult::Payload { client_id, payload } => {
            if let Err(e) = reliable_server.process_packet_from(payload, client_id) {
                println!("Error while processing payload for {}: {}", client_id, e);
            }
        }
